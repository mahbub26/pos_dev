...
class Db_model extends CI_Model
{
    ...
    {
        $this->db->order_by('id', 'desc');
        $q = $this->db->get_where("companies", array('group_name' => 'customer'), 10);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $this->db->order_by('id', 'desc');
        $q = $this->db->get_where("companies", array('group_name' => 'supplier'), 10);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getChartData()
    {
        ...
        COALESCE( R.expenses + P.purchases, 0 ) as totalcost,
        ...
        FROM (  SELECT  date_format(date, '%Y-%m') Month,
            ...
            LEFT JOIN ( SELECT  date_format(date, '%Y-%m') Month,
                        ...
                        GROUP BY date_format(date, '%Y-%m')) R
            ON S.Month = R.Month
    ...
    public function getTotalPosSales()
    {
        $this->db->select('count(id) as total, sum(COALESCE(grand_total, 0)) as total_pos_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
            ->where('sale_status !=', 'pending')->where('type =', 'sales');
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getTotalRepairSales()
    {
        $this->db->select('count(id) as total, sum(COALESCE(grand_total, 0)) as total_repair_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
            ->where('sale_status !=', 'pending')->where('type =', 'repair');
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getTotalReadyNotified()
    {
        $this->db->select('count(id) as id', FALSE)
            ->where('type_detail =', 'Notified as Ready');
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getTotalReadyNotifiedPaid()
    {
        $this->db->select('count(id) as id', FALSE)
            ->where('type_detail =', 'Notified as Ready')->where('payment_status =', 'paid');
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getTotalReadyNotifiedDue()
    {
        $this->db->select('count(id) as id', FALSE)
            ->where('type_detail =', 'Notified as Ready')->where('payment_status =', 'due');
        $q = $this->db->get('sales');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
